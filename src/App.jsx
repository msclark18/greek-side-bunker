...
.forgot-pw{font-size:.76rem;color:var(--cream-dim);text-align:right;cursor:pointer;display:block;background:none;border:none;width:100%;margin-top:-6px}
...
.user-chip{display:flex;align-items:center;gap:8px;background:var(--navy-card);border:1px solid var(--navy-border);border-radius:40px;padding:5px 12px 5px 5px}
...
input[type=text],input[type=email],input[type=number],input[type=date],select{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:8px;padding:10px 12px;color:var(--cream);font-family:var(--font-b);font-size:.92rem;transition:border-color .2s;width:100%;-webkit-appearance:none}
...
.modal{background:var(--navy-card);border:1px solid var(--gold-border);border-radius:14px;padding:30px;width:100%;max-width:480px;margin:16px;box-shadow:0 24px 80px rgba(0,0,0,.5);animation:fadeUp .25s ease;max-height:90vh;overflow-y:auto}
...
export default function App() {
  ...
  useEffect(() => {
    if (!session) { setProfile(null); setLeagues([]); setActiveLeague(null); return; }
  ...
  const myApprovedOnCourse = (cid) => rounds.filter(r =>
    r.player_id === session?.user.id && r.course_id === cid &&
